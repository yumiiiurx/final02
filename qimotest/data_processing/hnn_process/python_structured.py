...
def repair_program_io(code):
    """
    修复程序输入输出格式的问题，并返回修复后的代码和代码列表。

    Args:
        code (str): 待修复的代码字符串。

    Returns:
        tuple: 包含修复后的代码和代码列表的元组。
    """

    # 正则表达式模式，用于匹配不同情况下的输入输出标记
    patterns = [
        re.compile(r"In ?\[\d+\]: ?"),  # case1 输入标记
        re.compile(r"Out ?\[\d+\]: ?"),  # case1 输出标记
        re.compile(r"( )+\.+: ?"),  # case1 连续行标记
        re.compile(r">>> ?"),  # case2 输入标记
        re.compile(r"\.\.\. ?")  # case2 连续行标记
    ]

    lines = code.split("\n")
    lines_flags = [0 for _ in range(len(lines))]  # 存储每行代码的标记

    code_list = []  # 存储修复后的代码列表
    ...
    if not bool_repaired:
        # 如果不是典型情况，则仅移除每个 Out 后的空标记行（0-flag）
        repaired_code = ""
        sub_block = ""
...
PATTERN_VAR_EQUAL = r"[a-zA-Z_][a-zA-Z0-9_, ]*="
PATTERN_VAR_FOR = r"for .+ in"
...
def PythonParser(code):
    """
    解析给定的 Python 代码，提取变量名称。

    Args:
        code (str): 待解析的 Python 代码字符串。
